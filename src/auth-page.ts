...
export function renderAuthPage(options: AuthPageOptions): string {
  ...
  const upstreamName = escapeHtml(options.upstreamName ?? "CXM");
  ...
  const notice = options.error
    ...
    : options.success
      ? `<div class="notice success">Đăng nhập thành công. MCP đã sẵn sàng gọi ${upstreamName}.</div>`
      : "";
  const content = options.authenticated
    ? `<div class="status"><span class="dot"></span><strong>Đã đăng nhập ${upstreamName}</strong></div>
       <p>Access token được giữ trong bộ nhớ và sẽ tự gia hạn bằng refresh token.</p>
       <label>Địa chỉ kết nối MCP</label>
       <code>${mcpUrl}</code>
       <form method="post" action="${logoutUrl}">
         <button class="secondary" type="submit">Đăng xuất ${upstreamName}</button>
       </form>`
    : `<form method="post" action="${actionUrl}" autocomplete="on">
         ...
         <button type="submit">Đăng nhập ${upstreamName}</button>
       </form>
       <p class="hint">Mật khẩu chỉ được chuyển qua HTTPS tới máy chủ ${upstreamName} để đổi token; MCP không lưu mật khẩu.</p>`;
...
</style></head><body><main class="card"><div class="brand">${instanceName}</div><h1>Đăng nhập để cấp quyền cho MCP</h1><p class="hint">Môi trường ${upstreamName}: <strong>${upstreamHost}</strong></p>${notice}${content}</main></body></html>`;
}
